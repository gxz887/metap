...
<body>
						...
						<c:forEach var="var" items="${userModel.items}">
							...
							<tr onmouseover="this.className='over'" onmouseout="this.className='out'"  onclick="showInfo(${ var.id },this);">
								<td><input  type="radio" name="selectNos" id="radioId${var.id}"  value="${ var.id }" /></td>
										...
										<td><div><c:out value="${var.password}" escapeXml="true"/></div></td>
										<td><div><c:out value="${var.role}" escapeXml="true"/></div></td>
								<td><a href="<%=request.getContextPath()%>/user/goView?dataId=${ var.id }" >查看</a>&nbsp;&nbsp;<a href="<%=request.getContextPath()%>/user/goEdit?dataId=${ var.id }" >编辑 &nbsp;&nbsp;<a href="<%=request.getContextPath()%>/user/doDelete?dataId=${ var.id }" >删除</a></a></td>
							</tr>
						</c:forEach>
					</table>
				</td>
    ...
    function goCreate(){
        ...
        form.action = "<%=request.getContextPath()%>/user/goCreate";
        form.submit();
    }
